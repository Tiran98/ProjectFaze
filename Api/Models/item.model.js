...
Item.updatedById = (id, item, result) => {
    sql.query(
        "UPDATE items SET item_name = ?, item_stock = ?, item_category = ?, unit_price = ?, item_description = ?, item_image = ? WHERE id = ?", [item.item_name, item.item_stock, item.item_category, item.unit_price, item.item_description, item.item_image, id],
        (err, res) => {
            if (err) {
                console.log("error: ", err);
                result(null, err);
