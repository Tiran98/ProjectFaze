...
OrderHistory.updatedById = (id, orderHistory, result) => {
    sql.query(
        "UPDATE order_history SET buyer_id = ?, seller_id = ?, item_id = ?, item_name = ?, item_qty = ?, item_category = ?, unit_price = ?, total_price = ? WHERE id = ?", [orderHistory.buyer_id, orderHistory.seller_id, orderHistory.item_id, orderHistory.item_name, orderHistory.item_qty, orderHistory.item_category, orderHistory.unit_price, orderHistory.total_price, id],
        (err, res) => {
            if (err) {
                console.log("error: ", err);
                result(null, err);
